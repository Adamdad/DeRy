...
@PIPELINES.register_module()
class RandLabel(object):
    def __init__(self, magnitude=0.1, num_classes=10) -> None:
        super().__init__()
        self.magnitude = magnitude
        self.num_classes = num_classes

    def rand_smooth_label(self, one_hot_like_label):
        assert self.num_classes > 0
        num_sample = one_hot_like_label.shape[0]
        mag = np.random.rand(num_sample) * self.magnitude
        smooth_label = one_hot_like_label * (1 - mag)
        smooth_label += mag / self.num_classes
        return smooth_label

    def one_hot(self, a):
      return np.squeeze(np.eye(self.num_classes)[a.reshape(-1)])

    def __call__(self, results):
        gt_label = results['gt_label']
        onehot_gt_label = self.one_hot(gt_label)
        results['gt_label'] = self.rand_smooth_label(onehot_gt_label)
        return results
